...
pub.getThemesWithAssets = function(callback, filterParam) {
    ...
    var query  = 'SELECT *, s.id as asset_id, t.id as theme_id, UNIX_TIMESTAMP(date_to) as timestamp_to FROM `themes` t';
    ...
    conn.query(query, function(err, themes, fields) {
        ...
        if (err) {  util.log(err); }
        var asset_domain = CONF["aws"]["thumbsCDNDomain"];

        //-- Calculate thumbnail URLs.
        _.map(themes, function(theme, key) {
          theme.thumb_url = '//' + asset_domain + '/thumbs/' + 'medium' + '/images/' + theme.asset_id + '.jpeg';
          return theme;
        });
