...
exports.browse = function(req, res) {
    ...
    async.parallel({
        ...
        submissions: function(callback) {
            exports.getAllByTheme(theme_id, callback);
        }
...
exports.show = function(req, res) {
    var submission_id = req.param('submission_id');

    var site_title = "Show Submission";
    ...
    async.parallel({
        ...
        submission: function(callback) {
            exports.load(submission_id, callback);
        }
    }, function(err, results){
        context.themes = results.themes;
        context.submission = results.submission;

        res.render('submission_show', context);
    });
}
...
exports.saveForm = function(req, res) {
    ...
    async.waterfall([
        function (callback) {
            exports.save(submission, callback);
        },
...
exports.load = function(submission_id, callback) {
    var conn = database.getConnection();

    var query  = 'SELECT s.id, s.contact_name, s.contact_email, ';
    query += 'a.id as asset_id, a. url, a.caption, a.copyright, a.location  FROM `assets` a ';
    query += 'JOIN `submissions` s ON a.submission_id = s.id '
    query += 'WHERE s.id = ?';

    conn.query(query, [submission_id], function(err, submissions) {
        ...
        submission.large_crop_url = '/thumbs/large/images/' + submission.asset_id + '.jpeg';
        callback(err, submissions[0]);
    });
}

exports.getAllByTheme = function (theme_id, callback) {
    var conn = database.getConnection();

    var query  = 'SELECT s.id, s.contact_name, s.contact_email, ';
        query += 'a. url, a.caption, a.copyright, a.location, a.id as asset_id  FROM `assets` a ';
        query += 'JOIN `submissions` s ON a.submission_id = s.id '
    ...
    conn.query(query, [theme_id], function(err, assets) {
        ...
        assets = _.map(assets, function(asset, key) {
          asset.thumb_url = mod_assets.thumbURLByID(asset.asset_id);
          return asset;
        });
...
exports.save = function(submission, callback) {
