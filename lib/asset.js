...
/**
 * Render a thumbnail URL for an image with a specified ID and preset.
 * @param asset_id
 * @param preset_name - defaults to 'medium'
 * @return {String}
 */
exports.thumbURLByID = function(asset_id, preset_name) {
  var presetName = preset_name || 'medium';

  var assetDomain = CONF["aws"]["thumbsCDNDomain"];
  return '//' + assetDomain + '/thumbs/' + presetName + '/images/' + asset_id + '.jpeg';
}

/**
 * Look up image URL by a unique ID. This is the decodeFn function used for connect-thumbs middleware
 *
 * @param encodedURL
 */
...
exports.getAllAssets = function(callback) {
    ...
    var query  = 'SELECT * FROM `assets` a';

    conn.query(query, function(err, assets, fields) {
